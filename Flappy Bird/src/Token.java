...
public class Token{
     ...
     * @param up  image file name for token that is going up.
     * @param down image file name for token that is going down.
     * @param neutral image file name for token that is going straight
     */
    public Token(String up, String down, String neutral){
        String src = new File("").getAbsolutePath()+"/src/";
        try{
          tokenUp = new ImageIcon( (src+up) );
          height = tokenUp.getIconHeight();

        }catch(Exception e){
            System.out.println("no file found: "+src+up);
            System.exit(-1);
        }
        try{
          tokenDown = new ImageIcon( (src+down) );
        }catch(Exception e){
            System.out.println("no file found: "+src+down);
            System.exit(-1);
        }
        try{
    ...
    public Token(String file){
        ...
        fileN = src+file; 
        try{
          tokenImage = new ImageIcon(fileN); 
          tokenUp = tokenImage;
          tokenDown = tokenUp;
          height = tokenImage.getIconHeight();
          width = tokenImage.getIconWidth();
        }catch(Exception e){
            System.out.println("no file found: "+fileN);
            System.exit(-1);
        }
    }   
